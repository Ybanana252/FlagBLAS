...
_CHPR_CONFIGS = [
    ...
    triton.Config({"BLOCK_M": 4, "BLOCK_N": 128}, num_warps=8, num_stages=1),
]


def _prune_chpr_configs(configs, named_args, **kwargs):
    # A single row per program wins for matrices fitting in one 128-column
    # tile. Autotune's restore hooks can obscure this small-kernel difference.
    n = named_args.get("N", kwargs.get("N"))
    if n <= 128:
        return [config for config in configs if config.kwargs["BLOCK_M"] == 1]
    return configs
...
def _chpr_tiled_jit(
    ...
    # One scalar inverse-triangle calculation per program, shared by its rows.
    ...
    update_r = alpha * (xr[:, None] * xc_r[None, :] + xi[:, None] * xc_i[None, :])
    update_i = alpha * (xi[:, None] * xc_r[None, :] - xr[:, None] * xc_i[None, :])
...
chpr_mthreads_kernel = libentry()(
    triton.autotune(
        ...
        prune_configs_by={"early_config_prune": _prune_chpr_configs},
    )(_chpr_tiled_jit)
)
...
def chpr(
    ...
    with torch_device_fn.device(AP.device):
        chpr_mthreads_kernel[_chpr_grid(n)](
