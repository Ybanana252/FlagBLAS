...
# Ascend-only regressions keep their platform guard on each test so the shared
# SYR2 module remains usable by other backends.
ASCEND_SYR2_DTYPES = [
    pytest.param(torch.float32, marks=pytest.mark.ssyr2),
    pytest.param(torch.complex64, marks=pytest.mark.csyr2),
]


def _syr2_ascend_to_device(tensor):
    if tensor.is_complex():
        return torch.view_as_complex(torch.view_as_real(tensor).to("npu"))
    return tensor.to("npu")


def _syr2_ascend_to_cpu(tensor):
    if tensor.is_complex():
        return torch.view_as_complex(torch.view_as_real(tensor).cpu())
    return tensor.cpu()


def _check_ascend_syr2(n, uplo, dtype, incx=1, incy=1, padding=0, offset=0, alpha=None):
    complex_input = dtype == torch.complex64
    op = flag_blas.csyr2 if complex_input else flag_blas.ssyr2
    assert "runtime/backend/_ascend/ops/syr2.py" in inspect.getsourcefile(op)
    if alpha is None:
        alpha = -0.75 + 0.625j if complex_input else -0.75
    lda = n + padding
    generator = torch.Generator().manual_seed(20260921 + n)
    x = torch.randn(n * incx, dtype=dtype, generator=generator)
    y = torch.randn(n * incy, dtype=dtype, generator=generator)
    a = 0.1 * torch.randn(n * lda, dtype=dtype, generator=generator)
    high = torch.complex128 if complex_input else torch.float64
    xv, yv = x[::incx].to(high), y[::incy].to(high)
    triangle = torch.ones((n, n), dtype=torch.bool)
    triangle = triangle.tril() if uplo == 0 else triangle.triu()
    expected = a.reshape(n, lda).clone()
    for start in range(0, n, 128):
        end = min(start + 128, n)
        update = alpha * (
            xv[start:end, None] * yv[None, :] + yv[start:end, None] * xv[None, :]
        )
        block = expected[start:end, :n]
        selected = triangle[start:end]
        block[selected] = (block.to(high) + update)[selected].to(dtype)
    # Nonzero offsets vary pointer alignment; trailing guards catch overwrites.
    a_storage = torch.full((offset + n * lda + 16,), 42, dtype=dtype)
    a_storage[offset : offset + n * lda] = a
    x_storage = torch.full((offset + x.numel() + 16,), 43, dtype=dtype)
    y_storage = torch.full((offset + y.numel() + 16,), 44, dtype=dtype)
    x_storage[offset : offset + x.numel()] = x
    y_storage[offset : offset + y.numel()] = y
    ad, xd, yd = map(_syr2_ascend_to_device, (a_storage, x_storage, y_storage))
    op(
        uplo,
        n,
        alpha,
        xd[offset : offset + x.numel()],
        incx,
        yd[offset : offset + y.numel()],
        incy,
        ad[offset : offset + n * lda],
        lda,
    )
    actual = _syr2_ascend_to_cpu(ad)
    got = actual[offset : offset + n * lda].reshape(n, lda)
    lhs, rhs = (
        (torch.view_as_real(got), torch.view_as_real(expected))
        if complex_input
        else (got, expected)
    )
    torch.testing.assert_close(lhs, rhs, rtol=1.3e-6, atol=2.6e-6)
    untouched = torch.ones((n, lda), dtype=torch.bool)
    untouched[:, :n] = ~triangle
    assert torch.equal(got[untouched], a.reshape(n, lda)[untouched])
    assert torch.equal(actual[:offset], a_storage[:offset])
    assert torch.equal(actual[offset + n * lda :], a_storage[offset + n * lda :])
    assert torch.equal(_syr2_ascend_to_cpu(xd), x_storage)
    assert torch.equal(_syr2_ascend_to_cpu(yd), y_storage)


@pytest.mark.parametrize("dtype", ASCEND_SYR2_DTYPES)
@pytest.mark.parametrize("uplo", [0, 1])
@pytest.mark.parametrize("n", [1, 17, 127, 512, 513, 1024, 2047, 2051])
@pytest.mark.syr2
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
def test_ascend_syr2_boundaries(n, uplo, dtype):
    _check_ascend_syr2(n, uplo, dtype)


@pytest.mark.parametrize("dtype", ASCEND_SYR2_DTYPES)
@pytest.mark.parametrize("uplo", [0, 1])
@pytest.mark.parametrize("n", [17, 129, 1025])
@pytest.mark.syr2
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
def test_ascend_syr2_stride_padding_offset(n, uplo, dtype):
    _check_ascend_syr2(n, uplo, dtype, incx=2, incy=3, padding=3, offset=1)


@pytest.mark.parametrize("dtype", ASCEND_SYR2_DTYPES)
@pytest.mark.syr2
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
def test_ascend_syr2_fresh_buffers_scalars_and_stream(dtype):
    # Same specialization, different addresses/scalars, on a non-default stream.
    # Neither input values nor the stream may be captured by the launcher cache.
    stream = torch.npu.Stream()
    with torch.npu.stream(stream):
        for alpha in (0, 1, -2, torch.tensor(0.5)):
            _check_ascend_syr2(65, 1, dtype, alpha=alpha)
        if dtype == torch.complex64:
            for alpha in (1j, -0.25 + 2j, torch.tensor(0.5 + 0.75j)):
                _check_ascend_syr2(65, 1, dtype, alpha=alpha)
    stream.synchronize()
...
@pytest.mark.syr2
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
def test_ascend_syr2_empty_and_validation(dtype):
    op = flag_blas.csyr2 if dtype == torch.complex64 else flag_blas.ssyr2
    empty = _syr2_ascend_to_device(torch.empty(0, dtype=dtype))
    op(0, 0, 1, empty, 1, empty, 1, empty, 1)
    with pytest.raises(AssertionError):
        op(0, 0, 1, empty, 0, empty, 1, empty, 1)
    with pytest.raises(AssertionError):
        op(2, 0, 1, empty, 1, empty, 1, empty, 1)
    with pytest.raises(AssertionError):
        op(0, 1, 1, empty, 1, empty, 1, empty, 1)


@pytest.mark.syr2
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
@pytest.mark.csyr2
def test_ascend_csyr2_rejects_unresolved_conjugate():
    x = _syr2_ascend_to_device(torch.tensor([1 + 2j, 3 - 4j]))
    a = _syr2_ascend_to_device(torch.zeros(4, dtype=torch.complex64))
    # Raw pointer launching must not silently ignore PyTorch's lazy conj bit.
    with pytest.raises(RuntimeError, match="resolved conjugate"):
        flag_blas.csyr2(0, 2, 1, x.conj(), 1, x, 1, a, 2)


@pytest.mark.parametrize("n", [1537, 2304, 2559, 3583, 3584, 3585, 4609])
@pytest.mark.parametrize("uplo", [0, 1])
@pytest.mark.syr2
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
@pytest.mark.csyr2
def test_ascend_csyr2_tuned_boundaries(n, uplo):
    _check_ascend_syr2(n, uplo, torch.complex64)


@pytest.mark.syr2
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
@pytest.mark.csyr2
def test_ascend_csyr2_tuned_fresh_scalars_stream_and_alignment():
    stream = torch.npu.Stream()
    with torch.npu.stream(stream):
        for alpha in (1j, -0.25 + 2j, torch.tensor(0.5 + 0.75j)):
            _check_ascend_syr2(1537, 1, torch.complex64, alpha=alpha)
        _check_ascend_syr2(2304, 0, torch.complex64, offset=1)
        _check_ascend_syr2(2304, 1, torch.complex64, incx=2, incy=3, padding=3, offset=1)
    stream.synchronize()
...
@pytest.mark.skipif(flag_blas.vendor_name != "ascend", reason="Ascend backend")
@pytest.mark.csyr2
def test_ascend_csyr2_8192_special_scalar(uplo):
    _check_ascend_syr2(8192, uplo, torch.complex64, alpha=-0.5 + 0.75j)
